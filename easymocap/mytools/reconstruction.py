...
def batch_triangulate(keypoints_, Pall, keypoints_pre=None, lamb=1e3):
    ...
    v = (keypoints_[:, :, -1]>0).sum(axis=0) # 每个keypoints置信因子大于0的视角数
    valid_joint = np.where(v > 1)[0] # 至少要有两个置信因子大于0的视角，才做3维，否则置零
    keypoints = keypoints_[:, valid_joint]
    conf3d = keypoints[:, :, -1].sum(axis=0)/v[valid_joint] # 有效关节中视角置信因子的平均值 
...
def simple_recon_person(keypoints_use, Puse):
    ...
    square_diff = (keypoints_use[:, :, :2] - kpts_repro[:, :, :2])**2 ## 投影后偏移error
