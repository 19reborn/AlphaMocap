...
class CritMinJoints(BaseCrit):
    ...
    def __call__(self, keypoints, bbox, **kwargs):
...
class CritBbox(BaseCrit):
    def __init__(self, log, **kwargs) -> None:
        super().__init__(log)
        self.max_area = 150*150
        self.max_width = 150
        self.max_height = 200
        #self.min_area = 
        self.min_width = 10
    ...
    def __call__(self, keypoints, bbox, **kwargs) -> bool:
        ...
        width = fabs(r-l)
        height = fabs(t-b)
        if width>self.max_width or height > self.max_height or width*height > self.max_area or width < self.min_width or height<self.min_height:
            return False
        else:
            return True
    def __str__(self) -> str:
...
class ComposedFilter:
    ...
    def __call__(self, keypoints, bbox, **kwargs) -> bool:
        ...
        for filt in self.filters:
            if not filt(keypoints, bbox, **kwargs):
                return False
        return True
