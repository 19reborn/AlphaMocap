...
class LossKeypointsMV2D(LossRepro):
    def __init__(self, keypoints2d, bboxes, Pall, cfg, scale=1.) -> None:
        """
        Args:
    ...
    def __call__(self, kpts_est, **kwargs):
        ...
        new_kpts_est = kpts_est.clone()

        new_kpts_est[:,:,:3] = new_kpts_est[:,:,:3] * self.scale
        kpts_homo = torch.cat([new_kpts_est[..., :self.nJoints, :], self.kpt_homo], dim=2)
