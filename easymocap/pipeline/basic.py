...
def multi_stage_optimize(body_model, params, kp3ds, kp2ds=None, bboxes=None, Pall=None, weight={}, cfg=None, scale = 1.):
    ...
    if kp2ds is not None:
        with Timer('Optimize 2D Pose/{} frames'.format(kp3ds.shape[0])):
            ...
            params = optimizePose2D(body_model, params, bboxes, kp2ds, Pall, weight=weight, cfg=cfg, scale = scale)
    return params

def multi_stage_optimize2d(body_model, params, kp2ds, bboxes, Pall, weight={}, args=None, scale = 1.):
    cfg = Config(args)
...
def smpl_from_keypoints3d2d(body_model, kp3ds, kp2ds, bboxes, Pall, config, args,
    weight_shape=None, weight_pose=None, scale = 1.0 ):
    ...
    params = multi_stage_optimize(body_model, params, kp3ds, kp2ds, bboxes, Pall, weight_pose, cfg, scale = scale)
    return params
...
def smpl_from_keypoints2d(body_model, kp3ds, kp2ds, bboxes, Pall, config, args, 
    weight_shape=None, weight_pose=None, scale = 1.):
    model_type = body_model.model_type
    params_init = body_model.init_params(nFrames=1)
    if weight_shape is None:
        weight_shape = load_weight_shape(model_type, args.opts)
    ...
    if weight_pose is None:
        weight_pose = load_weight_pose(model_type, args.opts)
    # We divide this step to two functions, because we can have different initialization method
    params = multi_stage_optimize2d(body_model, params, kp2ds, bboxes, Pall, weight_pose, args, scale= scale)
    return params